...
def acumular_estadisticas_dia(
    ...
    """Suma a estadisticas[dia][protocolo] los registros, conversaciones totales y sospechosas de un lote.
    Solo guarda contadores, de modo que las conversaciones pueden liberarse tras la llamada."""

    estadisticas_dia = estadisticas.setdefault(dia, {})
    acumulado = estadisticas_dia.setdefault(protocolo, {"registros": 0, "conversaciones": 0, "sospechosas": 0, "desordenados": 0})
    acumulado["registros"] += contadorRegistrosFlujo
    acumulado["conversaciones"] += len(conversaciones)
    acumulado["sospechosas"] += sum(1 for conv in conversaciones if es_sospechosa(conv))
    acumulado["desordenados"] += contadorFlujosDesordenados

def imprimir_estadisticas_por_dia(estadisticas: dict[datetime.date, dict[str, dict[str, int]]], ruta_archivo_resultado: str) -> None:
    """Escribe por día y protocolo los registros, conversaciones, sospechosas y su porcentaje."""

    archivoOutput = os.path.join(ruta_archivo_resultado, ARCHIVO_ESTADISTICAS_POR_DIA)
    ...
    def formatear(nombre: str, protocolo: str, valores: dict[str, int]) -> str:
        ...
        porcentaje_desordenados = valores["desordenados"] / valores["registros"] * 100 if valores["registros"] else 0
        return f"{nombre:<12} {protocolo:<6} {valores['registros']:>14} {valores['conversaciones']:>16} {valores['sospechosas']:>13} {porcentaje:>13.4f}% {valores['desordenados']:>14} {porcentaje_desordenados:>14.4f}%"

    for dia in sorted(estadisticas):
        for protocolo in protocolos:
            valores = estadisticas[dia].get(protocolo, {"registros": 0, "conversaciones": 0, "sospechosas": 0, "desordenados": 0})
            lineas.append(formatear(dia.isoformat(), protocolo, valores))
            for clave in totales[protocolo]:
                totales[protocolo][clave] += valores[clave]

    lineas.append("-" * len(lineas[0]))
    for protocolo in protocolos:
        lineas.append(formatear("TOTAL", protocolo, totales[protocolo]))
    ...
    print(f"\n{contenido}\nEstadísticas por día guardadas en {archivoOutput}")

def imprimir_resultado(conversaciones: list[Conversacion], ruta_archivo_resultado: str, aniadir: bool = False) -> None:
    """Escribe las conversaciones sospechosas en TXT, sustituyendo o añadiendo al resultado existente."""

    archivoOutput = os.path.join(ruta_archivo_resultado, ARCHIVO_RESULTADO)

    modo = "a" if aniadir else "w"
    with open(archivoOutput, modo, encoding="utf-8", buffering=1024 * 1024) as f:
        if not aniadir:
