...
def leer_registros_flujo_ordenados(
...
) -> Iterator[tuple[float, list[bytes], bool]]:
    """
    ...
    buffer: list[tuple[float, int, list[bytes], bool]] = [] # registros de flujo que todavía no se pueden emitir, como tuplas (timestamp, orden, columnas, desordenado) para mantener el orden de llegada en caso de timestamps iguales
    ...
    with open(ruta_archivo, "rb") as f:
        ...
        with mmap.mmap(f.fileno(), length=0, access=mmap.ACCESS_READ) as mm:
            ...
            while True:
                ...
                flujo_desordenado = timestamp < ultimo_emitido
                if flujo_desordenado:
                    print(
                ...
                heapq.heappush(buffer, (timestamp, orden, cols, flujo_desordenado)) #heapq mantiene el buffer ordenado por timestamp y luego por orden de llegada
                orden += 1
                ...
                while buffer and buffer[0][0] <= marca_de_agua: #estos registros de flujo ya no pueden ser adelantados por ninguno posterior
                    timestamp_emitido, _, cols_emitidas, flujo_desordenado = heapq.heappop(buffer)
                    ultimo_emitido = timestamp_emitido
                    yield timestamp_emitido, cols_emitidas, flujo_desordenado
