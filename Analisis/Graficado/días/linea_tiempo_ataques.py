"""Dibuja todos los ataques de resultado.txt en una línea de tiempo,
basándose en su tiempo de inicio y final.
"""
...
def generarGraficoLineaTiempoAtaques(archivo=ARCHIVO_ENTRADA, directorioGuardado=None, mostrar=True, numFigura=1):
    ...
    plt.figure(numFigura, figsize=(14, 3))
    ejes = plt.gca()
    ...
    segmentos = [((xi, 0), (xf, 0)) for xi, xf in zip(x_inicio, x_fin)]
    ...
    ejes.scatter(x_inicio, [0] * len(x_inicio), color=COLOR_ATAQUE, s=8, zorder=3)

    ejes.set_xlim(min(x_inicio), max(x_fin))
    ejes.set_ylim(-1, 1)
    ejes.set_yticks([])
    ejes.set_title(f"Attack timeline ({len(ataques_ordenados)} attacks)")
